	...
	while (run) {
		...
		if (pbkd->cap.read(grab)) {
			...
			if (pbkd->frame > 0 && pbkd->cap.set(cv::CAP_PROP_POS_FRAMES, 0)) {
				...
				if (debug)
					fprintf(stderr, "background: thread stopping at end of stream and not resettable\n");
...
std::shared_ptr<background_t> load_background(const std::string& path, int debug) {
	...
	try {
		...
		if (!pbkd->cap.isOpened()) {
			if (pbkd->debug)
				fprintf(stderr, "background: cap cannot open: %s\n", path.c_str());
		...
		if (pbkd->cap.read(pbkd->raw) && pbkd->cap.read(pbkd->raw)) {
			...
			if (pbkd->raw.empty()) {
				if (pbkd->debug)
					fprintf(stderr, "background: imread cannot open: %s\n", path.c_str());
